...
require ("db.php");
if( $_POST ){
    $query = "SELECT * FROM previous_orders GROUP BY orderid order by deletTime desc";
    $result = mysqli_query($con,$query);
        while ($row = $result -> fetch_row()) {
          $serial=$row[0];
            ...
            $address=$row[5];
            $time=$row[7];
            echo "<tr id='$orderID-tr'><td id='$orderID-ss' class='serial'>$serial</td><td id='$orderID-id' class='idd'>$orderID</td><td id='$orderID-na' class='name'>$name</td><td id='$orderID-ord' class='orders'>$Orderz</td><td id='$orderID-ph' class='phone'>$phone</td><td id='$orderID-ad' class='address'>$address</td><td id='$orderID-ti' class='time'>$time</td><td><form  action='permanentdelete.php'><input type='submit' id='$orderID' class='button' value='Delete'></form></td></tr>";
        }
}
    ?>
</table>
...
<script> 
   ...
   function fun($oID){
      ...
      address: $address,
      time: $time,
      previous:1 //prev 1 means its previous order checking 0 means active table in database
    },
   ...
   //
    $("form").on("submit", function(event){
        ...
        var formValues= $(this).serialize()+"&orderID="+$orderID;
